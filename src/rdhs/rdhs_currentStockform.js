...
class rdhsCurrentStockForm extends Component {
  ...
  handleChange(event) {
    ...
    if(name=='r_store_id'){
      const rdhsstores=this.state.rdhsstores;
      const store=rdhsstores.find(ms => ms.r_store_id==target.value);
      item[name]=store;
      this.setState({item});
    }else if(name=='sr_no'){
  ...
  render() {
    const {item,rdhsstores,isLoading,medicines,exportedstocks} = this.state;
    const title = <h2>{'Add Group'}</h2>;
    

    
    const groupList = rdhsstores.map(rdhsstore => {
      return <option 
                key={rdhsstore.r_store_id} 
                value={rdhsstore.r_store_id}>
            {rdhsstore.location}
            </option>
    });
      ...
      <div className="animated fadeIn">        
        <Row>
          <Col>
            <Card>
              <CardHeader>
                <strong> RDHS Current Stock</strong> Form
              </CardHeader>
              <CardBody>
                ...
                <FormGroup row>
                    ...
                    <Col xs="12" md="9">                  
                      <Input type="select" name="r_store_id" id="stock_id" value={item.r_store_id.r_store_id|| ''} onChange={this.handleChange}>
...
export default rdhsCurrentStockForm;
