...
import {FormGroup,Table, Button, Card,Input,Col,Row,InputGroup,InputGroupAddon} from 'reactstrap';
import { Link } from 'react-router-dom';

class LM_Hospital_SortBy_Qty extends Component {
  constructor(props) {
    super(props);
    this.state = { 
      reg_no:'',
  ...
  async componentDidMount(){
    const response= await fetch('/dhlessqty/'+this.state.reg_no);
  ...
  render() { 
    const {Drug} =this.state;
      let filteredData=Drug.filter((drugs) => {
    ...
    return ( 
      <form>
        <FormGroup>         
          ...
          <Link to='/dhordercart'><Button color="primary">View  Cart</Button></Link>
        </FormGroup> 
        <Row>
          <Col>
            <InputGroup>
...
export default LM_Hospital_SortBy_Qty;