...
class SupplyToDHModel extends Component {
  ...
  componentDidMount() {
    this.setState({isLoading: true});
      ...
      fetch(`/rdhsreqorderdetail/${orderId}`)
      .then(response => response.json())
  ...
  handleChange(event) {
    ...
    let item = {...this.state.item};
     if(name=='track_id'){
      ...
      const rdhsreqorderdetail=this.state.rdhsreqorderdetail;
      console.log('rdhsreqorderdetail',rdhsreqorderdetail);
      item[name]=rdhsreqorderdetail;
      this.setState({item});
      console.log('item',item);
    }else{
  ...
  async handleSubmit(event) {
    ...
    await fetch('/supplyordertordhs/add', {
      method:'POST',
      headers: {
  ...
  render() {
    const {item,ministrycurrentstocks,ministrytracks,rdhsreqorderdetail} = this.state;
    ...
    return (
      <div className="animated fadeIn">        
          ...
          <ModalHeader toggle={this.props.toggle}>Accept request from Rdhs</ModalHeader>
          <ModalBody>
            <Card>
              <CardHeader>
